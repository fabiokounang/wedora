(function () {
  ...
  function renderWishes() {
    ...
    sorted.forEach((w) => {
      ...
      div.className = 'bg-cream border border-sand rounded-2xl p-4';
      div.innerHTML = `<p class="font-serif text-deep mb-1">${escHtml(w.guest_name)}</p><p class="text-brown text-sm">${escHtml(w.message)}</p>`;
  ...
  function animateNavTap(link) {
    link.classList.remove('nav-link-clicked'); void link.offsetWidth;
  ...
  document.querySelectorAll("#navbar a[href^='#'], #mobile-menu a[href^='#']").forEach((a) => {
  ...
  document.querySelectorAll('.copy-acct').forEach((btn) => {
    btn.addEventListener('click', async () => {
      ...
      try {
        ...
        const toast = g('copy-toast'); if (toast) { toast.classList.remove('hidden'); setTimeout(() => toast.classList.add('hidden'), 2000); }
      } catch (_) {}
    });
  });
