(function () {
  ...
  function openInvitation() {
    ...
    setTimeout(function () {
      ...
      if (main) {
        main.classList.remove('t14-main--hidden');
  ...
  function renderWishes() {
    ...
    if (!sorted.length) {
      ...
      p.id = 'noWishes';
      p.className = 'text-center text-sm text-amber-700/45 italic';
      p.textContent = 'Jadilah yang pertama meninggalkan ucapan hangat.';
      list.appendChild(p);
    } else {
      sorted.forEach(function (w) {
        const div = document.createElement('div');
        div.className = 'card-romantic rounded-xl p-4 t14-wish-card animate-scale-in';
        div.innerHTML =
          (w.message
            ? '<p class="text-sm text-amber-900/70">&ldquo;' + escHtml(w.message) + '&rdquo;</p>'
            : '') +
          '<p class="text-xs text-amber-700/40 mt-1">\u2014 ' +
